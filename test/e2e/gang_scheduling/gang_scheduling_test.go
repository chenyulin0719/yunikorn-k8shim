...
var _ = Describe("", func() {
	...
	// 	var pods, _ = kClient.GetPodNamesFromNS(ns)
	// 	var jobs, _ = kClient.GetJobNamesFromNS(ns)
	// 	// var pods, _ = kClient.GetPodNamesFromNSAndStatus(ns)
	// 	// var jobs, _ = kClient.GetJobNamesFromNSAndStatus(ns)
	// 	By(fmt.Sprintf("### Before delet ns, pod count: %v , job: %v", len(pods), jobs))
	// 	time.Sleep(100 * time.Second)

	// 	// ns := "ns-test"
	// 	// By("Tear down namespace: " + ns)
	// 	// By(fmt.Sprintf("### TearDownNamespace Completed"))

	// 	err := kClient.TearDownNamespace(ns) //  pod 立馬被刪掉,  job還在
	// 	By(fmt.Sprintf("### call TearDownNamespace(),  err: %v ", err))

	// 	// err := kClient.DeleteNamespace(ns) //  pod 過幾秒才被刪掉,  job還在
	// 	// By(fmt.Sprintf("### call DeleteNamespace(),  err: %v ", err))

	// 	// add a for loop, each loop will sleep 1 sec
	// 	for i := 0; i < 10; i++ {
	// 		time.Sleep(2 * time.Second)
	// 		pods, _ = kClient.GetPodNamesFromNS(ns)
	// 		jobs, _ = kClient.GetJobNamesFromNS(ns)
	// 		// pods, _ = kClient.GetPodNamesFromNSAndStatus(ns)
	// 		// jobs, _ = kClient.GetJobNamesFromNSAndStatus(ns)
	// 		By(fmt.Sprintf("### After delet ns, pod count: %v , job: %v", len(pods), jobs))
	// 	}
	// 	// pods, _ = kClient.GetPodNamesFromNS(ns)
	// 	// By(fmt.Sprintf("### After delete  %v ,", pods))

	// 	By(fmt.Sprintf("### End of test"))
	// })
	...
	AfterEach(func() {
		...
		if testDescription.Failed() {
			By("Dumping cluster status to artifact directory...")
